...
begin
  ...
  try
    ...
    cd.AddChild(TFHIRElementDefn.create(cMandatory, 1, 1, '', '', bsUnspecified, 'issued', 'date issed for current status', 'The date and/or time that the result was issued from the source for the recorded ?Test result status', 'Clinicians need to be able to check the date that the report was released', true, 'unique(./participation[isHighest(priorityCode) and typeCode="AUT" and isNormalParticipation()]/time[type="TS"])', 'May be different from LabReport.updated, because that is the status of this record, not the report the record is about', '', '', 'isHighest is a short-cut that indicates that, in the event there are multiple occurrences that match the remaining constraints, the data element is the one with the highest non-null value for the indi'+
      'cated property.'+#10+''+#10+'isNormalParticipation is a short-cut meaning:'+#10+'not(negationInd="true" or subsetCode[not(nullFlavor="NA")])'+#10+''+#10+'Question: Said this should be highest priority.  Is that appropriate?', '', '', 'instant', false));
