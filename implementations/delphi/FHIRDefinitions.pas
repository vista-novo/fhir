...
procedure addConceptDomainResourceProfileStatus(definitions : TFHIRDefinitions);
var
  cd : TFHIRConceptDomain;
begin
  cd := TFHIRConceptDomain.create();
  try
    cd.name := 'ResourceProfileStatus';
    cd.definition := 'The lifecycle status of a Resource Profile';
    cd.bindingType := btCodeList;
    cd.binding := 'resource-profile-status';
    cd.details := '';

    cd.codes.add(TFHIRDefinedCode.create('draft', 'This profile is still under development', ''));
    cd.codes.add(TFHIRDefinedCode.create('testing', 'this profile was authored for testing purposes (or education/evaluation/evangelisation)', ''));
    cd.codes.add(TFHIRDefinedCode.create('production', 'This profile is ready for use in production systems', ''));
    cd.codes.add(TFHIRDefinedCode.create('withdrawn', 'This profile has been withdrawn', ''));
    cd.codes.add(TFHIRDefinedCode.create('superceded', 'This profile was superceded by a more recent version', ''));
    definitions.conceptDomains.add(cd.Link);
  finally
    cd.free
  end;
...
procedure addConceptDomainRace(definitions : TFHIRDefinitions);
var
  cd : TFHIRConceptDomain;
begin
  cd := TFHIRConceptDomain.create();
  try
    cd.name := 'Race';
    cd.definition := 'The race of the person';
    cd.bindingType := btPreferred;
    cd.binding := 'HL7 v2 table 0200';
    cd.details := '';
...
procedure addConceptDomainConceptBindingType(definitions : TFHIRDefinitions);
var
  cd : TFHIRConceptDomain;
begin
  cd := TFHIRConceptDomain.create();
  try
    cd.name := 'ConceptBindingType';
    cd.definition := 'The type of a binding for a concept domain';
    cd.bindingType := btCodeList;
    cd.binding := 'concept-binding-type';
    cd.details := '';

    cd.codes.add(TFHIRDefinedCode.create('Unbound', 'The concept domain is not bound to anything', ''));
    cd.codes.add(TFHIRDefinedCode.create('CodeList', 'The concept domain is bound to a list of supplied codes - only those codes are allowed', ''));
    cd.codes.add(TFHIRDefinedCode.create('Reference', 'The concept domain references some external definition by a provided reference', ''));
    cd.codes.add(TFHIRDefinedCode.create('Preferred', 'The concept domain references a set of preferred terms', ''));
    cd.codes.add(TFHIRDefinedCode.create('Suggestion', 'This profile was superceded by a more recent version', ''));
    cd.codes.add(TFHIRDefinedCode.create('External', 'The concept domain is defined by an external authority identified in the reference', ''));
    definitions.conceptDomains.add(cd.Link);
  finally
    cd.free
  end;
...
begin
  ...
  try
    ...
    cd.AddChild(TFHIRElementDefn.create(cConditional, 0, 1, '', '', bsUnspecified, 'profile', 'Resource Profile that supplies the constraints', 'Reference to a resource profile which includes the constraint statement that applies to this resource', '', false, '', '', '', '', '', 'Provide either a profile reference or constraints on the resource elements', '', 'uri', false));
    cd.AddChild(TFHIRElementDefn.create(cOptional, 0, 1, '', '', bsUnspecified, 'name', 'Name for this Constraint Statement', 'The name of this aggregation profile', '', false, '', 'This field is the target for a named aggregation definition (aggregation.name)', '', '', '', '', '', 'string', false));
    ...
    cd.AddChild(TFHIRElementDefn.create(cConditional, 0, -1, '', '', bsUnspecified, 'element', '', '', '', false, '', '', '', '', '', 'Provide either a profile reference or constraints on the resource elements', '', '', false)
      ...
      .AddChild(TFHIRElementDefn.create(cOptional, 0, -1, '', '', bsUnspecified, 'mapping', '', '', '', false, '', '', '', '', '', '', '', '', false))
        ...
        .AddChild(TFHIRElementDefn.create(cOptional, 0, 1, '', '', bsUnspecified, 'profile', 'Reference to a Resource Profile', 'Reference to a Resource Profile', '', false, '', 'The reference might be a simple name in the form #[name], which is a reference to another named constraint in a resource - where the resource contains more than one constraint, or a RUL which is a reference to a resource Profile', '', '', '', '', '', 'uri', false))
      .AddChild(TFHIRElementDefn.create(cConditional, 0, 1, '', '', bsUnspecified, 'resource', 'If context includes aggregation and type=Resource()', 'If context includes aggregation and type=Resource()', '', false, '', '', '', '', '', 'If context includes aggregation and type=Resource()', '', '', false))
...
begin
  ...
  try
    ...
    cd.AddChild(TFHIRElementDefn.create(cOptional, 0, -1, '', '', bsUnspecified, 'profile', 'other profiles that apply (i.e. code binding rules)', 'Additional other profiles that apply to this conformance statement.', '', false, '', ' Generally these profiles make  constraints on the bindings of concept domains / value sets', '', '', '', '', '', 'uri', false));
...
procedure addResourceElementDefnProfile(definitions : TFHIRDefinitions);
var
  cd : TFHIRElementDefn;
begin
  cd := TFHIRElementDefn.create(cMandatory, 1, 1, '', '', bsUnspecified, 'Profile', 'Resource Profile', 'A Resource Profile - a statement of constraint on one or more Resources and/or Concept Domains', '', false, '', 'Often called a clinical template', '', '', '', '', '', 'Resource', false);
  try
    cd.AddChild(TFHIRElementDefn.create(cMandatory, 1, 1, '', '', bsUnspecified, 'id', 'fixed identifier for this profile (UUID)', 'Globally unique Profile Id (Must be a UUID)', '', true, '', '', '', '', '', '', '', 'id', false));
    cd.AddChild(TFHIRElementDefn.create(cMandatory, 1, 1, '', '', bsUnspecified, 'name', 'Informal name for this profile', 'A free text natural language name identifying the Template.', '', false, '', 'Not expected to be unique', '', '', '', '', '', 'string', false));
    cd.AddChild(TFHIRElementDefn.create(cMandatory, 1, 1, '', '', bsUnspecified, 'author', '(Organisation or individual)', 'Details of the author who accepts responsibility for publishing the profile', '', false, '', 'Usually an organisation, but may be an individual', '', '', '', '', '', '', false)
      .AddChild(TFHIRElementDefn.create(cMandatory, 1, 1, '', '', bsUnspecified, 'name', 'Name of the recognised author', 'The name of the author', '', false, '', '', '', '', '', '', '', 'string', false))
      .AddChild(TFHIRElementDefn.create(cOptional, 0, -1, '', '', bsUnspecified, 'reference', 'Web reference to the author', 'Reference to the author to assist a user in finding and communicating with the author', '', false, '', 'may be a web site, an email address, a telephone number', '', '', '', '', '', 'uri', false)));
    cd.AddChild(TFHIRElementDefn.create(cOptional, 0, 1, '', '', bsUnspecified, 'intention', 'Why this profile was written', 'A free text natural language description of the intent and scope of the Template. The purpose is to provide the author?s initial intent for the Template in the language specified below.', '', false, '', '', '', '', '', '', '', 'string', false));
    cd.AddChild(TFHIRElementDefn.create(cOptional, 0, -1, '', '', bsUnspecified, 'code', 'assist with indexing and finding', 'A set of terms from a controlled reference terminology that may be used to assist with indexing and searching of templates', '', false, '', '', '', '', '', '', '', 'Coding', false));
    cd.AddChild(TFHIRElementDefn.create(cMandatory, 1, 1, '', '', bsUnspecified, 'description', 'natural language description of the Template', 'A free text natural language description of the Template. Generally, this field should be used for things such as goals, variable lists, instructions for clinical use and interpretation, literature, e'+
      'xamples from paper world, mapping from natural language to HL7 and the model itself, etc', '', false, '', '', '', '', '', '', '', 'string', false));
    cd.AddChild(TFHIRElementDefn.create(cOptional, 0, -1, '', '', bsUnspecified, 'evidence', 'Supporting evidence for the design', 'A description, reference or link to a published medical knowledge that was used in the definition of this Template', '', false, '', '', '', '', '', '', '', 'uri', false));
    cd.AddChild(TFHIRElementDefn.create(cOptional, 0, 1, '', '', bsUnspecified, 'comments', 'Guidance for use, usage notes, etc', 'A statement regarding when this Template should not be used, or may be used erroneously. To define roles where the Template should not be used, or should be used with care. This field is used to expand in detail on the iIntention', '', false, '', '', '', '', '', '', '', 'string', false));
    cd.AddChild(TFHIRElementDefn.create(cMandatory, 1, 1, '', 'ResourceProfileStatus', bsUnspecified, 'status', 'draft | testing | production | withdrawn | superceded', 'draft | testing | production | withdrawn | superceded', '', true, '', '', '', '', '', '', '', 'code', false));
    cd.AddChild(TFHIRElementDefn.create(cMandatory, 1, 1, '', '', bsUnspecified, 'date', 'date for given status', 'The date that the current value for publicationStatus was applied to the Template', '', false, '', '', '', '', '', '', '', 'dateTime', false));
    cd.AddChild(TFHIRElementDefn.create(cOptional, 0, -1, '', '', bsUnspecified, 'endorser', '(Organisation or individual)', 'A list of bodies who have reviewed the Template for clinical accuracy and relevance, and endorsed it for use.', '', false, '', '', '', '', '', '', '', '', false)
      .AddChild(TFHIRElementDefn.create(cMandatory, 1, 1, '', '', bsUnspecified, 'name', 'Name of the recognised endorser', 'The name of the endorsing body', '', false, '', '', '', '', '', '', '', 'string', false))
      .AddChild(TFHIRElementDefn.create(cOptional, 0, 1, '', '', bsUnspecified, 'reference', 'Web reference to the endorser', 'Reference to the author to assist a user in finding and communicating with the endorsing body', '', false, '', '', '', '', '', '', '', 'uri', false)));
    cd.AddChild(TFHIRElementDefn.create(cConditional, 0, 1, '', '', bsUnspecified, 'changes', 'notes about changes since last version', 'A free text description describing the changes in this version of the profile as compared to the previous version. ', '', false, '', '', '', '', '', 'This field must be populated if the template is not the first version.', '', 'string', false));
    cd.AddChild(TFHIRElementDefn.create(cOptional, 0, -1, '', '', bsUnspecified, 'supercedes', 'references to previous versions', 'A template that is supercded by this template (maybe a  previous version)', '', false, '', '', '', '', '', '', '', 'uri', false));
    cd.AddChild(TFHIRElementDefn.create(cOptional, 0, -1, '', '', bsUnspecified, 'profile', 'other profiles that apply (i.e. code binding rules)', 'Additional other profiles that apply to this conformance statement.', '', false, '', ' Generally these profiles make  constraints on the bindings of concept domains / value sets', '', '', '', '', '', 'uri', false));
    cd.AddChild(TFHIRElementDefn.create(cMandatory, 0, -1, '', '', bsUnspecified, 'resource', 'Resource Type with constraints', 'Resource Type with constraints', '', false, '', '', '', '', '', '', '', 'Constraint', false));
    cd.AddChild(TFHIRElementDefn.create(cMandatory, 0, -1, '', '', bsUnspecified, 'binding', '', '', '', false, '', '', '', '', '', '', '', '', false)
      .AddChild(TFHIRElementDefn.create(cMandatory, 1, 1, '', '', bsUnspecified, 'name', 'concept domain name', 'The name of the concept domain that this profile is declaring a constraint on', '', false, '', '', '', '', '', '', '', 'string', false))
      .AddChild(TFHIRElementDefn.create(cMandatory, 1, 1, '', 'ConceptBindingType', bsUnspecified, 'type', 'binding type', 'The form of the binding', '', false, '', '', '', '', '', '', '', 'code', false))
      .AddChild(TFHIRElementDefn.create(cConditional, 0, 1, '', '', bsUnspecified, 'details', 'extra details - see notes', 'extra details - see notes', '', false, '', '', '', '', '', 'May be required depending on the value of binding.type', '', 'string', false))
      .AddChild(TFHIRElementDefn.create(cConditional, 0, 1, '', '', bsUnspecified, 'reference', 'source of binding content', 'source of binding content', '', false, '', '', '', '', '', 'May be required depending on the value of binding.type', '', 'uri', false))
      .AddChild(TFHIRElementDefn.create(cConditional, 0, -1, '', '', bsUnspecified, 'code', 'enumerated codes that are the binding', 'enumerated codes that are the binding', '', false, '', '', '', '', '', 'May be required depending on the value of binding.type', '', 'Coding', false)));
    cd.AddChild(TFHIRElementDefn.create(cOptional, 0, 1, '', '', bsUnspecified, 'extensions', 'See Extensions', 'See Extensions', '', false, '', '', '', '', '', '', '', '', false));
    cd.AddChild(TFHIRElementDefn.create(cMandatory, 1, 1, '', '', bsUnspecified, 'text', 'Text summary of resource profile for human interpretation', 'Text summary of resource profile for human interpretation', '', false, '', '', '', '', '', '', '', 'Narrative', false));
    definitions.resources.add(cd.Link);
  finally
    cd.free
  end;
...
begin
  ...
  try
    ...
    cd.AddChild(TFHIRElementDefn.create(cOptional, 0, -1, '', 'Race', bsUnspecified, 'race', 'The race of a person', 'blah balh', 'This is added because many jurisdictions insisted on it', false, '.raceCode', 'Some places do not use this!', '', '', '', '', '', 'CodeableConcept', false));
...
begin
  cd := TFHIRElementDefn.create(cMandatory, 1, 1, '', '', bsUnspecified, 'LabReport', '', 'The findings and interpretation of pathology tests performed on tissues and body fluids. This is typically done in a laboratory but may be done in other environments such as at the point of care', 'Use to record any pathology test result, including the result of a test on a specimen taken as part of a composite procedure or operation.', false, 'Observation[isNormalAct() and subsumesCode("ActClass#OBS", classCode) and moodCode="EVN" and domainMember("LabObservationType", code)]', 'Not to be used for reporting on non-pathology test results e.g. diagnostic imaging, ECG or respiratory function tests. Not to be used to represent an entire cumulative report. This Pathology test resu'+
      ...
      'multiple OBSERVATION archetypes. This archetype is suitable for representation of general pathology test results, but not intended to cover full synoptic reports. For these, additional resources are required to represent the data properly', '', '', 'isNormalAct is a short-cut meaning:'+#10+'not(actionNegationInd="true" or isCriterionInd="true")', '', '', 'Resource', false);
  try
    cd.AddChild(TFHIRElementDefn.create(cMandatory, 1, 1, '', '', bsUnspecified, 'id', 'Master Resource Id, always first in all resources', 'Master Resource Id, always first in all resources', '', false, 'unique(./id[isNormalDatatype() and displayable="false" and scope="OBJ" and reliability="ISS"])', '', '', '', 'unique is a short-cut indicating that a match only occurs in the event that there is only one match for the specified path and set of constraints.  If multiple values are found, the mapping does not a'+
      'pply.'+#10+''+#10+'isNormalDatatype is a short-cut meaning: (validTimeLow<=today() or not(validTimeLow)) and (validTimeHigh>=today() or not(validTimeHigh)) and not(nullFlavor)', '', '', 'id', false));
    cd.AddChild(TFHIRElementDefn.create(cMandatory, 1, 1, '', 'LabReportStatus', bsUnspecified, 'status', 'Registered|Interim|Final|Amended|Cancelled|Withdrawn', 'The status of the pathology test result as a whole', 'Labs routinely issue provisional/incomplete reports, or withdraw previously released reports', true, 'registered: ./statusCode[isNormalDatatype()]="new";'+#10+''+#10+'interim: ./statusCode[isNormalDatatype()]="active";'+#10+''+#10+'final: ./statusCode[isNormalDatatype()]="complete" and not(./inboundRe'+
      'lationship[typeCode="SUBJ" and isNormalActRelationship()]/source[subsumesCode("ActClass#CACT") and moodCode="EVN" and domainMember("ReviseLabOrder", code) and isNormalAct()]);'+#10+''+#10+'amended: ./'+
      'statusCode[isNormalDatatype()]="complete" and ./inboundRelationship[typeCode="SUBJ" and isNormalActRelationship()]/source[subsumesCode("ActClass#CACT") and moodCode="EVN" and domainMember("ReviseLabOrder", code) and isNormalAct() and statusCode="completed"];'+#10+''+#10+'cancelled: '+#10+'./statusCode[isNormalDatatype()]="aborted";'+#10+''+#10+'withdrawn: ./statusCode[isNormalDatatype()]="obsolete"', '', '', '', 'isNormalActRelationship is a short-cut meaning:'+#10+'not(inversionInd="true" or negationInd="true" or subsetCode[not(nullFlavor="NA")])', '', '', 'code', false));
    cd.AddChild(TFHIRElementDefn.create(cMandatory, 1, 1, '', '', bsUnspecified, 'issued', 'date issed for current status', 'The date and/or time that the result was issued from the source for the recorded ?Test result status', 'Clinicians need to be able to check the date that the report was released', true, 'firstOf(unique(./participation[isHighest(priorityCode) and typeCode="VRF" and isNormalParticipation()]/time[type="TS"]), '+#10+'unique(./participation[isHighest(priorityCode) and typeCode="AUT" and isNormalParticipation()]/time[type="TS"]))', 'May be different from LabReport.updated, because that is the status of this record, not the report the record is about', '', '', 'isHighest is a short-cut that indicates that, in the event there are multiple occurrences that match the remaining constraints, the data element is the one with the highest non-null value for the indi'+
      'cated property.'+#10+''+#10+'isNormalParticipation is a short-cut meaning:'+#10+'not(negationInd="true" or subsetCode[not(nullFlavor="NA")])'+#10+''+#10+'firstOf is a short-cut meaning that the semant'+
      'ics could come from multiple places, but the most precise semantic is listed first.  (This generally means that the value for the first semantic is assumed to default to the value of a later semantic if not explicitly stated.)'+#10+''+#10+'Question: Said this should be highest priority.  Is that appropriate?', '', '', 'instant', false));
    cd.AddChild(TFHIRElementDefn.create(cMandatory, 1, 1, '', '', bsUnspecified, 'patient', 'The patient the report is about', 'The patient about who the report is about', 'Must know the patient context', true, 'unique(./participation[isHighest(priorityCode) and typeCode="SBJ" and isNormalParticipation()]/role[classCode="PAT"])', '', '', 'need to check the wording -is it about, upon, for?', 'Question: Said this should be highest priority.  Is that appropriate?', '', '', 'Resource(Patient)', false));
    cd.AddChild(TFHIRElementDefn.create(cOptional, 0, 1, '', '', bsUnspecified, 'admission', 'Admission Context', 'The admission that this diagnostic investigation is associated with', 'Some institutions track and file diagnostic reports under a specific admission', false, 'unique(./inboundRelationship[isHighest(priorityCode) and typeCode="SUBJ" and isNormalActRelationship()]/source[subsumesCode("ActClass#ENC", code) and moodCode="EVN" and isNormalAct()])', '', '', 'Reckon this is not 80%', 'Question: Said this should be highest priority.  Is that appropriate?', '', '', 'Resource(Admission)', false));
    cd.AddChild(TFHIRElementDefn.create(cMandatory, 1, 1, '', '', bsUnspecified, 'laboratory', 'Responsible Laboratory', 'The laboratory service that issued the report', 'Need to know how to contact if there are queries about the results. Also may need to track the source of reports for secondary data analysis', false, 'unique(./participation[isHighest(priorityCode) and typeCode="AUT" and isNormalParticipation()]/role[subsumesCode("RoleClass#ASSIGN", classCode)]/scoper[subsumesCode("EntityClass#ORG", classCode) and determinerCode="INSTANCE"])', 'This is not necessarily the source of the atomic reports - it''s the lab that takes responsibility for the clinical report', '', '', 'Question: Said this should be highest priority.  Is that appropriate?', '', '', 'Resource(Organization)', false));
    cd.AddChild(TFHIRElementDefn.create(cOptional, 0, 1, '', '', bsUnspecified, 'reportId', 'Id for external references to this report', 'The local ID assigned to the report by the order filler, usually by the Laboratory Information System (LIS).', 'Need to know what identifier to use when making queries about this report from the source laboratory', false, 'unique(./id[displayable="true" and scope="OBJ"])', '', '', '', '', '', '', 'id', false));
    cd.AddChild(TFHIRElementDefn.create(cUnstated, 0, -1, '', '', bsUnspecified, 'requestDetail', '', 'Details concerning a single pathology test requested.', 'Need to be able to track completion of requests based on reports issued, and also to report what diagnostic test swere requested (not always the same as what is delivered)', false, 'unique(./outboundRelationship[typeCode="FLFS" and isNormalActRelationship()]/target[subsumesCode("ActClass#OBS", classCode) and subsumes("ActMood#PRMS", moodCode) and isNormalAct()])', 'Note: Usually there is one test request for each result, however in some circumstances multiple test requests may be represented using a single Pathology test result resource', '', '', 'Question: Should this be by priorityCode too?', '', '', '', false)
      .AddChild(TFHIRElementDefn.create(cOptional, 0, 1, '', '', bsUnspecified, 'requestOrderId', 'Id assigned by requester', 'The local ID assigned to the order by the order requester.', 'Need to be able to track completion of requests based on reports issued', false, 'unique(./outboundRelationship[typeCode="FLFS" and isNormalActRelationship()]/target[subsumesCode("ActClass#OBS", classCode) and subsumes("ActMood#RQO", moodCode) and isNormalAct()]/id[scope="OBJ" and displayable="true"])', 'Equivalent to the Placer Order Identifier', '', 'Reckon this is not 80%', 'Question: Should this be by priorityCode too?', '', '', 'Identifier', false))
      .AddChild(TFHIRElementDefn.create(cOptional, 0, 1, '', '', bsUnspecified, 'receiverOrderId', 'Receiver''s Id for the request', 'The local ID assigned to the test order by the order filler, usually by the Laboratory Information System (LIS).', 'Need to be able to track completion of requests based on reports issued', false, 'unique(./id[scope="OBJ" and displayable="true"])', 'Usually equivalent to the DICOM Accession Number and the Filler Order Identifier.', '', 'Reckon this is not 80%', '', '', '', 'Identifier', false))
      .AddChild(TFHIRElementDefn.create(cUnstated, 0, -1, '', 'LabRequests', bsUnspecified, 'requestTest', 'Test Requested', 'Identification of pathology test requested,', 'Need to be able to report what diagnostic test swere requested (not always the same as what is delivered)', false, './code', 'Useful where the test requested differs from the test actually performed.', '', '', '', '', '', 'CodeableConcept', false))
      .AddChild(TFHIRElementDefn.create(cOptional, 0, 1, '', '', bsUnspecified, 'requester', 'Responsible for request', 'Details of the clinician or organisation requesting the laboratory test.', 'The requesting clinician may need to be contacted concerning the interpretation of the lab report', false, 'Agent: unique(./participation[isHighest(priorityCode) and typeCode="AUT" and isNormalParticipation()]/role[subsumesCode("RoleClass#ASSIGN", classCode)]'+#10+''+#10+'Organization: unique(./participatio'+
      'n[isHighest(priorityCode) and typeCode="AUT" and isNormalParticipation()]/role[subsumesCode("RoleClass#ASSIGN", classCode)]/scoper[subsumesCode("EntityClass#ORG", classCode) and determinerCode="INSTANCE" and code[isNormalDatatype]])'+#10+'', '', '', '', '', '', '', 'Resource(Agent|Organization)', false))
      .AddChild(TFHIRElementDefn.create(cOptional, 0, 1, '', '', bsUnspecified, 'clinicalInfo', 'Clinical information provided', 'Details of the clinical information provided to the laboratory along with the original request', 'Knowing the clinical information may influence the interpretation of the result', false, './outboundRelationship[typeCode="PERT" and isNormalActRelationship()]/target', '', '', 'This isn''t really *ANY* resource.  You can''t send a drug or an Animal or a bunch of other things.  What you really want here are "acts", and I suspect there''s a constraint that the subject of the L'+
      'abReport must have participated in them as subject or perhaps in some other way.', '', '', '', 'Resource(Any)', false)));
    cd.AddChild(TFHIRElementDefn.create(cMandatory, 1, 1, '', 'LabReportNames', bsUnspecified, 'reportName', 'Name for the entire report', 'Identification of the pathology test performed, sometimes including specimen type.', 'Need to know what report this is, so clinicians can filter/find the reports they are looking for', true, './code', 'A test result may be for a single analyte, or a group of items, including panel tests.', '', '', '', '', '', 'CodeableConcept', false));
    cd.AddChild(TFHIRElementDefn.create(cOptional, 0, 1, '', 'LabServices', bsUnspecified, 'service', 'Biochemistry, Haematology etc', 'The diagnostic service that performs the examination e.g. biochemistry, haematology.', 'Help clinicians filter/find the reports they are looking for', false, 'unique(./inboundRelationship[typeCode="COMP" and isNormalActRelationship()]/source[subsumesCode("ActClass#LIST", classCode) and moodCode="EVN" and inDomain(code,"Lab Service") and isNormalAct()]/code', '', '', 'I''m not sure this is in the 80%.', 'Could also model this via observation', '', '', 'CodeableConcept', false));
    cd.AddChild(TFHIRElementDefn.create(cMandatory, 1, 1, '', '', bsUnspecified, 'diagnosticTime', 'Effective time of diagnostic report', 'The diagnostically relevant time for this report', 'Need to know where in the patient history to file/present this report', true, './effectiveTime[type="TS"]', 'The diagnostically relevant time can be derived from the specimen collection times, but the specimen information is not always available, and the exact relationship is not always automatic', '', '', '', '', '', 'dateTime', false));
    cd.AddChild(TFHIRElementDefn.create(cUnstated, 0, -1, '', '', bsUnspecified, 'specimen', 'Specimen (incl. time of collection)', 'Details about the specimen if all individual test results are derived from the same specimen', 'Need to be able to report information about the collected specimens on which the report is based', false, 'unique(./participation[typeCode="SUBJ" and isNormalParticipation()]/role[subsumesCode("RoleClass#SPEC", classCode)])', 'If the specimen is sufficiently specified with a code in the Test result name, then this additional data may be redundant. If there are multiple specimens, these may be represented per ''Result group''', '', '', '', '', '', 'Resource(Specimen)', false));
    cd.AddChild(TFHIRElementDefn.create(cUnstated, 1, -1, '', '', bsUnspecified, 'resultGroup', '', 'A group of results. Results may be grouped by specimen, or by some value in LabReport.resultGroup.name to describe what binds all the results together.', 'Need to be able to report groups of results, where the result grouping is arbitrary, but meaningful', true, './outboundRelationship[typeCode="COMP" and isNormalActRelationship()]/target[isNormalAct() and subsumesCode("ActClass#OBS", classCode) and moodCode="EVN" and fromDomain(code, "LabObservationBattery")]', 'Many (most) lab reports don''t really have a meaningful group. In these cases, just create a single group with no specimen or name', '', '', '', '', '', '', false)
      .AddChild(TFHIRElementDefn.create(cOptional, 0, 1, '', 'LabResultGroupNames', bsUnspecified, 'name', 'What defines the group', 'A code or name that describes the group of results', '', true, './code', 'For example, the antibody code for a goup of antibody related test, or the organism code for a group of isolate/sensitivities. Leave blank if there is no particular meaning associated with the group', '', '', '', '', '', 'CodeableConcept', false))
      .AddChild(TFHIRElementDefn.create(cOptional, 0, 1, '', '', bsUnspecified, 'specimen', 'Specimen details', 'Details about the individual specimen to which these ?Result group? test results refer, where testing of multiple specimens is required.', 'Need to be able to report information about the collected specimens on which the report is based', false, 'unique(./participation[typeCode="SBJ" and isNormalParticipation()]/role[subsumesCode("RoleClass#SPEC", classCode)])', '', '', '', '', '', '', 'Resource(Specimen)', false))
        .AddChild(TFHIRElementDefn.create(cOptional, 0, 1, '', 'LabResultNames', bsUnspecified, 'name', 'Name or code of the result', 'Identifies the meaning of the value', 'Need to know what the result is about', true, './code', 'results are fundamentally a name - value pair with additional clarifying information', '', '', '', '', '', 'CodeableConcept', false))
        .AddChild(TFHIRElementDefn.create(cOptional, 0, 1, '', '*unbound*', bsUnspecified, 'value[x]', 'Result. [x] = type name', 'Actual value of the result. Most result values will be numerical measurements, but others may be coded concepts, free text, or multimedia images', 'Need a value for the result', true, './value', '', '', '', '', '', '', 'Quantity|CodeableConcept|Attachment|Ratio|Choice|Interval(dateTime)|string', false))
        .AddChild(TFHIRElementDefn.create(cOptional, 0, 1, '', 'LabResultFlag', bsUnspecified, 'flag', '+ | ++ | +++ | - | -- | --- ', 'Flag indicating the abnormal status of the result', '', true, './interpretationCode', '', '', 'The proposed set of codes are too narrow (and doesn''t map well to v3.)', '', '', '', 'code', false))
        .AddChild(TFHIRElementDefn.create(cMandatory, 1, 1, '', 'LabReportStatus', bsUnspecified, 'status', 'Registered|Interim|Final|Amended|Cancelled|Withdrawn', 'The status of the result value', 'Need to track the status of individual results - some results are finalised before the whole report is finalised', true, '(as per LabReport.status)', '', '', '', '', '', '', 'code', false))
        .AddChild(TFHIRElementDefn.create(cOptional, 0, 1, '', '', bsUnspecified, 'comments', 'Comments about result', 'May include statements about significant, unexpected or unreliable. values, or information about the source of the value where this may be relevant to the interpretation of the result.', 'Need to be able to provide free text additional information', true, 'unique(./inboundRelationship[typeCode="SUBJ" and isNormalActRelationship()]/source[subsumesCode("ActClass#OBS", classCode) and moodCode="EVN" and mapsTo(code, "LOINC#'+#10+'48767-8") and isNormalAct()]/value[type="ST"])', '', '', '', 'mapsTo is a short-cut indicating that any code that maps to the specified code or any code subsumed by the specified code may be used.', '', '', 'string', false))
          .AddChild(TFHIRElementDefn.create(cOptional, 0, 1, '', 'LabReferenceRanges', bsUnspecified, 'meaning', 'The meaning of this range', 'Code for the meaning of the reference range', 'Need to be able to say what kind of reference range this is - normal, recommended, therapeutic, or perhaps what state this reference range applies to (i.e. hormonal cycles)', false, './interpretationCode', '', '', '', '', '', '', 'CodeableConcept', false))
          .AddChild(TFHIRElementDefn.create(cMandatory, 1, 1, '', '', bsUnspecified, 'range[x]', 'Reference', 'Actual value of the reference range.  May be a quantity (<20mg/L), an interval (10-20 umol/L), or some text', 'Need to be able to report numerical or text reference ranges, and handle legacy data', false, './value[type=("PQ", "IVL_PQ", "ST")]', 'Text reference ranges are typically used in endocrinology, or for legacy data with string reference ranges', '', '', '', '', '', 'Quantity|Interval(Quantity)|string', false))
        .AddChild(TFHIRElementDefn.create(cUnstated, 0, -1, '', '', bsUnspecified, 'referenceRange', '', 'Guidance on how to interpret the value by comparison to a normal or recommended range', 'Need to be able to provide multiple reference ranges', true, './outboundRelationship[typeCode="REFV" and isNormalActRelationship()]/target[subsumesCode("ActClass#OBS", classCode) and moodCode="EVN" and isCriterionInd="true" and isNormalAct()]', 'Most results only have one reference range. Some non-numerical results don''t have a reference range', '', 'Are multiple reference ranges in the 80%?', '', '', '', '', false))
      ...
      ' labs report as part of the clinical service; it is not confined to measurements.', 'Need to report results with information that assist with interpretation', true, './outboundRelationship[typeCode="COMP" and isNormalActRelationship()]/target[subsumesCode("ActClass#OBS", classCode) and moodCode="EVN" and isNormalAct() and fromDomain(code, "LabObservationResult")]', '', '', '', '', '', '', '', false)));
    cd.AddChild(TFHIRElementDefn.create(cOptional, 0, 1, '', '', bsUnspecified, 'conclusion', 'Clinical Interpretation of test results', 'Concise and clinically contextualised narrative interpretation of the pathology test results.', 'Need to be able to provide a conclusion that is not lost amongst the basic result data', true, 'unique(./inboundRelationship[typeCode="SPRT" and isNormalActRelationship()]/source[subsumesCode("ActClass#OBS", classCode) and moodCode="EVN" and mapsTo(code, "LOINC#'+#10+'48767-8") and isNormalAct()]/value[type="ST"])', 'common reports don''t have a conclusion, but some do', '', 'Is this in the 80%?', '', '', '', 'Narrative', false));
    cd.AddChild(TFHIRElementDefn.create(cUnstated, 0, -1, '', 'LabDiagnosisCodes', bsUnspecified, 'codedDiagnosis', 'Codes for the conclusion', 'Codes for the conclusion', '', false, 'unique(./inboundRelationship[typeCode="SPRT" and isNormalActRelationship()]/source[subsumesCode("ActClass#OBS", classCode) and moodCode="EVN" and subsumesCode(code, "LOINC#'+#10+'54531-9") and isNormalAct()]/value[type="CD"])', '', '', '', '', '', '', 'CodeableConcept', false));
    cd.AddChild(TFHIRElementDefn.create(cUnstated, 1, -1, '', '', bsUnspecified, 'representation', 'Entire Report as issued', 'Rich text representation of the entire result as issued by the diagnostic service. Multiple formats are allowed but they must be semantically equivalent.', 'Laboratory needs to be able to provide it''s own fully formatted report for clinical fidelity', true, './text[type="ED"]', 'Possible formats: text/html, text/plain, text/rtf, application/msword, application/pdf, application/rtf, application/vnd.oasis.opendocument.text, application/vnd.openxmlformats-officedocument.wordprocessingml.document', '', '', '', '', '', 'Attachment', false));
    cd.AddChild(TFHIRElementDefn.create(cOptional, 0, 1, '', '', bsUnspecified, 'extensions', 'See Extensions', 'See Extensions', '', false, 'N/A', '', '', '', '', '', '', '', false));
    cd.AddChild(TFHIRElementDefn.create(cMandatory, 1, 1, '', '', bsUnspecified, 'text', 'Text summary of report, for human interpretation', 'Text summary of report, for human interpretation', '', false, 'unique(./text[type="ST"])', '', '', '', '', '', '', 'Narrative', false));
    definitions.resources.add(cd.Link);
  finally
    cd.free
  end;
...
procedure addDefinedResourceElementDefnProfile(definitions : TFHIRDefinitions);
var
  cd : TFHIRElementDefn;
begin
  cd := TFHIRElementDefn.create(cMandatory, 1, 1, '', '', bsUnspecified, 'Profile', 'Resource Profile', 'A Resource Profile - a statement of constraint on one or more Resources and/or Concept Domains', '', false, '', 'Often called a clinical template', '', '', '', '', '', 'Resource', false);
...
begin
  ...
  try
    ...
    addConceptDomainResourceProfileStatus(result);
    ...
    addConceptDomainConceptBindingType(result);
