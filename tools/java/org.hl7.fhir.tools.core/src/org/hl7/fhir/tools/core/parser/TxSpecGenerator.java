...
public class TxSpecGenerator extends OutputStreamWriter {
	...
	public TxSpecGenerator(OutputStream out) throws UnsupportedEncodingException {
		super(out, "UTF-8");
	}

	public void generate(ElementDefn root, List<ConceptDomain> tx) throws Exception
	{
...
//						sids.put(sid, new DefinedCode())
				...
				if (bs == BindingStrength.Extensible)
...
//					write("    <tr><td><b>Code</b></td><td colspan=\"2\"><b>Title</b></td></tr>");
			...
			} else if (cd.getBindingType() == BindingType.Reference) {
			...
			} else if (cd.getBindingType() == BindingType.Preferred) {
			...
			} else if (cd.getBindingType() == BindingType.Suggestion) {
			...
			} else if (cd.getBindingType() == BindingType.External) {
			...
			} else if (cd.getBindingType() == BindingType.Special) {
	...
	private void scan(ElementDefn e, String path, List<ConceptDomain> tx) throws Exception {
	...
	private ConceptDomain getConceptDomainByName(List<ConceptDomain> tx, String conceptDomain) throws Exception {		
