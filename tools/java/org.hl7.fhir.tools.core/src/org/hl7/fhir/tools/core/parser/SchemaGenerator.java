...
public class SchemaGenerator extends OutputStreamWriter {
	...
	private List<ConceptDomain> tx;
	private Map<String, List<DefinedCode>> enums = new HashMap<String, List<DefinedCode>>();
	
	public SchemaGenerator(OutputStream out) throws UnsupportedEncodingException {
		super(out, "UTF-8");
	}
	...
	public void generate(ElementDefn root, List<ConceptDomain> tx) throws Exception
	{
		...
		//write("<!-- Generated using FHIR Schema Generator -->\r\n");
		...
		for (TypeDefn t : datatypes) {
			if (t.getName().equals("Resource"))
				...
				for (String p : t.getParams()) {
					write("       <xs:element name=\""+t.getName()+"_"+p+"\" type=\""+t.getName()+"_"+p+"\"/>\r\n");				
				}
			} else {
	...
	private String encodeType(ElementDefn e, TypeDefn type, boolean params) throws Exception {
		if ("Resource".equals(type.getName()))
			...
			return type.getName()+"_"+type.getParams().get(0);
	}
