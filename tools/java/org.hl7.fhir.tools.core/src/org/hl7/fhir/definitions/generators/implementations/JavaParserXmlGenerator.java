...
public class JavaParserXmlGenerator extends OutputStreamWriter {
  ...
  private void start(String version, String genDate) throws Exception {

    write("package org.hl7.fhir.instance.formats;\r\n");
    write("\r\n");
    write("// © HL7 (http://www.hl7.org)  Generated on "+genDate+" for FHIR v"+version+"\r\n");
    write("\r\n");
...
//    write("  protected "+tn+" parse"+tn.replace("<", "_").replace(">", "")+"(XmlPullParser xpp) throws Exception {\r\n");
  ...
  private void generate(ElementDefn n, boolean hasDataAbsentReason, boolean contentsHaveDataAbsentReason) throws Exception {
    typeNames.clear();
    typeNameStrings.clear();
    enums.clear();
    strucs.clear();
    ...
    genInner(n, hasDataAbsentReason, contentsHaveDataAbsentReason);
    
    for (ElementDefn e : strucs) {
      genInner(e, hasDataAbsentReason, contentsHaveDataAbsentReason);
    }

  }

  private void generateConstraint(DefinedCode cd, boolean hasDataAbsentReason, boolean contentsHaveDataAbsentReason) throws Exception {
    typeNames.clear();
    typeNameStrings.clear();
    enums.clear();
    strucs.clear();
...
//      if (!n.typeCode().equals("Resource") || (!e.getName().equals("id") && !e.getName().equals("extensions") && !e.getName().equals("text")))
  ...
  private void genInner(ElementDefn n, boolean hasDataAbsentReason, boolean contentsHaveDataAbsentReason) throws IOException, Exception {
    ...
    if (tn.contains(".")) {
      ...
      write("    "+tn+" res = owner.new "+pathNode(tn)+"();\r\n");
      bUseOwner = true;
    } else {
    ...
    for (ElementDefn e : n.getElements()) {
//      if (!n.typeCode().equals("Resource") || (!e.getName().equals("id") && !e.getName().equals("extensions") && !e.getName().equals("text"))) {
        genElement(n, e, first, contentsHaveDataAbsentReason, bUseOwner);
        first = false;
      //}
    }
    write("      } else\r\n");
  ...
  private void genElement(ElementDefn root, ElementDefn e, boolean first, boolean contentsHaveDataAbsentReason, boolean bUseOwner) throws Exception {
