...
public class CSVGenerator extends OutputStreamWriter {
	...
	{
		write("Element,Card.,Conf.,Type,Binding,Must Understand,Short Name,Definition,Requirements,Comments,RIM Mapping,v2 Mapping,To Do,Committee Notes\r\n");
	...
	private void generateElement(String name, ElementDefn e) throws IOException {
		write(name+"."+e.getName()+","+e.describeCardinality()+","+e.getConformance().fullName()+","+checkNull(describeType(e))+","+checkNull(e.getBindingName())+","+checkBoolean(e.isMustUnderstand())+","+e.getShortDefn()+","+checkNull(e.getDefinition())+","+checkNull(e.getRequirements())+","+checkNull(e.getComments())+","+checkNull(e.getRimMapping())+","+checkNull(e.getV2Mapping())+","+checkNull(e.getTodo())+","+checkNull(e.getCommitteeNotes())+"\r\n");
